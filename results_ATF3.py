...
def anomaly_detection_in_oil_temp(y_pred, y_true, window):
    ...
    return anomalies, MR, MRmean, UCL, LCL
...
anomalies, MR, MRmean, UCL, LCL = anomaly_detection_in_oil_temp(ypred, Y_test, 3)
plot_anomalies(anomalies, MR, MRmean, UCL, LCL)

#find ids marked abnormal by DGA
ids = data_find_abnormal_DGA(ATF3, OLMS_DATA_top_oil_mapping, DGA_mapping)
anomalies_from_DGA=pd.DataFrame(ids)
anomalies_from_DGA.set_index("Date", inplace=True)
