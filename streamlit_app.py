...
if 'OLMS_DATA_mapping' not in st.session_state:
    st.session_state.OLMS_DATA_mapping = {'Top Oil Temperature':None,'Ambient Temperature':None}
        ...
        with tabs_historical[1]:
            if st.session_state.OLMS_DATA is not None:
                ...
                for key in st.session_state.OLMS_DATA_mapping.keys():
                    option = st.selectbox(key+':', st.session_state.OLMS_DATA.Measurement.unique().tolist())
